...
import {DailyPuzzleIcon, DailyAssessmentIcon, MazeScapeIcon, CheckIcon, ProgressBrainIcon, TaskCompleteIcon} from '../../../public/assessment';
...
export default function VisualReasoningStaticAssessment() {
  ...
  const progressPercent = useMemo(
      () => Math.min(100, Math.round(((currentIndex + 1) / questions.length) * 100)),
  ...
  return (
      <>
        ...
        <div className="mx-auto px-4 lg:px-12 py-4 lg:py-7" style={{ fontFamily: 'Roboto, sans-serif' }}>
          <div className="flex flex-col lg:flex-row gap-4 h-auto lg:h-[580px]">
            ...
            <div className="lg:w-1/4 w-full bg-[#EEEEEE] rounded p-6 flex flex-col h-full">
              ...
              <p className="text-[13px] mb-4">Test your memory by finding all the matching pairs of cards.</p>

              {/* On small/medium screens → show current question only + toggle */}
              <div className="flex-grow flex flex-col">
                {/* Mobile/Tablet view */}
                <div className="block lg:hidden">
                  {/* Current active question box (always highlighted) */}
                        ...
                        {questions.map((q, index) => (
                            <div
                                ...
                                className={`p-3 rounded text-[13px] cursor-default border ${
                                    index === currentIndex ? 'bg-[#FFE2D9] border-[#FF2738]' : 'bg-white border-transparent'
                                }`}
                            >
                              {q.title}
                            </div>
                  ...
                  {currentQuestion.options.map((option) => (
                      <label
                          ...
                          className={`flex justify-between items-center p-2 border rounded cursor-pointer ${
                              selectedOption === option ? 'border-orange-400' : 'border-gray-300'
                          }`}
                      >
        ...
        {/* Completion Modal */}
        <AssessmentCompletionModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
      </>
  );
}
